...
from custome_inferrence_model import VisualizationDemo
...
def setup_cfg(config_path = "/media/binh/D/KLTN/data/train_outputs/aistron/maskrcnn/maskrcnn_R101_FPN_cocoa_8ep_bs2/config.yaml",
              default_dataset_name = "crystal_amodal_default",
              default_dataset_json = "/media/binh/D/KLTN/data/datasets/COCOA/annotations/annotations_aistron.json",
              default_image_dir = "/media/binh/D/KLTN/data/datasets/COCOA/annotations/",
              model_path = "/media/binh/D/KLTN/data/train_outputs/aistron/maskrcnn/maskrcnn_R101_FPN_cocoa_8ep_bs2/model_final.pth",
              threshold=0.5):
...
def inferrence(image,model_path, cfg, threshold=0.5):
    # Load the model
    cfg.MODEL.WEIGHTS = model_path
    cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = threshold
...
def handle_inferrence(image_path,model_path,default_dataset_name = "crystal_amodal_default",threshold=0.5):
    ...
    visible_image, amodal_image = inferrence(preprocessed_img,model_path, cfg, threshold=threshold)

    return image, visible_image, amodal_image

def test_API():
    default_dataset_name = "crystal_amodal_default"
    image_path = '/media/binh/D/KLTN/data/datasets/COCOA/annotations/PNGImages/_rotateN90_flipH_1.png'
    model_path = '/media/binh/D/KLTN/data/train_outputs/model_final.pth'
    ...
    cv2.imwrite('image_visible.jpg', visible_image)
    cv2.imwrite('image_amodal.jpg', amodal_image)
