...
COCOA_CATEGORIES = [
    {'color': RANDOM_COLOR, 'isthing': 1, 'supercategory': 'crystal', 'id': 1, 'name': 'crystal'}
]
...
def register_cocoa(root):
    if "cocoa_train_crystal" in DatasetCatalog.list():
        DatasetCatalog.remove("cocoa_train_crystal")

    register_aistron_cocolike_instances("cocoa_train_crystal", _get_cocoa_instances_meta(COCOA_CATEGORIES),
        join(root, "annotations_aistron.json"),
        join(root, "")
    )
    if "cocoa_val_crystal" in DatasetCatalog.list():
        DatasetCatalog.remove("cocoa_val_crystal")
    register_aistron_cocolike_instances("cocoa_val_crystal", _get_cocoa_instances_meta(COCOA_CATEGORIES),
        join(root, "annotations_aistron.json"),
        join(root, "")
    )
    if "cocoa_test_crystal" in DatasetCatalog.list():
        DatasetCatalog.remove("cocoa_test_crystal")
    register_aistron_cocolike_instances("cocoa_test_crystal", _get_cocoa_instances_meta(COCOA_CATEGORIES),
        join(root, "annotations_aistron.json"),
        join(root, "")
    )
